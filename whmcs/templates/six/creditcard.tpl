    ...
    <form method="post" action="creditcard.php" class="form-horizontal" role="form">
        ...
        <div class="row">
            <div class="col-md-7">
                ...
                {/if}
                {if $errormessage_ipara}
                    <div class="alert alert-danger">{$errormessage_ipara}</div>
                {/if}
                ...
                <div class="form-group">
					...
					{* iPara taksitler kodu başlıyor *}				
					{if isset($installments)}
                    <label for="ipara_installment" class="control-label">Taksit Seçimi</label>
					<select name="ipara_installment" class="form-control select-inline">
					{for $ins=1 to 9}
						{if $ins eq 1}
                            <option value="{$ins}"> Tek çekim {$installments.$ins.total} </option>
						{else}
                            <option value="{$ins}"> {$ins} taksit X {$installments.$ins.monthly} toplam {$installments.$ins.total} </option>
						{/if}
					{/for}
					</select>
					{/if}
					{* iPara taksitler kodu bitti	*}
