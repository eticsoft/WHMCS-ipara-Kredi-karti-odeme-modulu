                ...
                <div id="creditCardInputFields"{if $selectedgatewaytype neq "CC"} class="hidden"{/if}>
                    ...
                    <div id="existingCardInfo" class="row{if !$clientsdetails.cclastfour || $ccinfo eq "new"} hidden{/if}">
 					...
 					{if isset($installments)}
					  <div id="ipara_installment" class="row">
							<div class="class="col-sm-12">
		                        ...
		                        <div class="col-sm-6">
									...
									{for $ins=1 to 9}
										{if $ins eq 1}
											<option value="{$ins}"> Tek çekim {$installments.$ins.total} {$ipara_currency} </option>
										{else}
											<option value="{$ins}"> {$ins} taksit X {$installments.$ins.monthly} toplam {$installments.$ins.total} {$ipara_currency}</option>
										{/if}
									{/for}
									</select>
								</div>								
